    ...
    layui.extend({
        ...
        tableIns = table.render({
            ...
            , cols: [[   //列表数据
                ...
                , {fixed: 'right', title: '操作', toolbar: '#logInfoBar', width: 250, align: 'center',}
            ]]
        });
        //模糊查询
        ...
        table.on('tool(logInfoTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                ...
                layer.confirm('真的删除行 [ ' + data.loginname + ' ] 这个日志吗?', function (index) {
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/logInfo/deleteLogInfo?id=" + data.id, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    });
        ...
        function deleteBatch() {
            ...
            if (dataLength > 0) {
                ...
                //发送请求
                layer.confirm('真的删除选中的 [ ' + ids + ' ] 这些日志吗', function (index) {
                    //向服务端发送删除指令
