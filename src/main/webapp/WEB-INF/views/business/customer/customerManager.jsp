    ...
    <div class="layui-form-item">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="career"  autocomplete="off"
                           class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
            ...
            <div class="layui-inline" style="margin:5px 0 0 800px">
                <button type="button" class="layui-btn layui-btn-normal  layui-icon layui-icon-download-circle" id="doExport">
                    导出客户
                </button>
                <button type="button" class="layui-btn layui-btn-normal  layui-icon layui-icon-search" id="doSearch">
                    查询
                </button>
                <button type="reset" id="dataFrmResetBtn" class="layui-btn layui-btn-warm  layui-icon layui-icon-refresh">
                    重置
                </button>
            </div>
        </div>
    ...
    layui.extend({
        ...
        tableIns = table.render({
            ...
            , cols: [[   //列表数据
                ...
                , {fixed: 'right', title: '操作', toolbar: '#customerBar', width: 250, align: 'center',}
            ]],
        ...
        $("#doExport").click(function () {
            var params = $("#searchFrm").serialize()
            window.location.href = "${pageContext.request.contextPath}/stat/exportCustomer?" + params;
        });
        ...
        table.on('tool(customerTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                ...
                layer.confirm('真的删除行 [ ' + data.custname + ' ] 这个客户吗?', function (index) {
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/customer/deleteCustomer?identity=" + data.identity, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    });
        ...
        function deleteBatch() {
            ...
            if (dataLength > 0) {
                ...
                //发送请求
                layer.confirm('真的删除选中的 [ ' + ids + ' ] 这些客户吗', function (index) {
                    //向服务端发送删除指令
