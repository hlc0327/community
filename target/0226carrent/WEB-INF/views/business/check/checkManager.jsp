    ...
    layui.extend({
        ...
        tableIns = table.render({
            ...
            , cols: [[   //列表数据
                ...
                , {field: 'checkid', title: '检查单号', align: 'center', width: '180'}
                , {field: 'rentid', title: '出租单号', align: 'center', width: '100'}
                , {field: 'checkdate', title: '检查时间', align: 'center', width: '120'}
                , {field: 'problem', title: '存在问题', align: 'center', width: '150'}
                , {field: 'checkdesc', title: '问题描述', align: 'center', width: '120'}
                ...
                , {fixed: 'right', title: '操作', toolbar: '#checkBar', width: 250, align: 'center',}
            ]],
        ...
        table.on('tool(checkTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                ...
                layer.confirm('真的删除行 [ ' + data.checknumber + ' ] 这个检查单吗?', function (index) {
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/check/deletecheck?checknumber=" + data.checknumber, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    });
        ...
        function openUpdatecheck(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${pageContext.request.contextPath}/check/updateCheck";
                }
            });
        }
